...
void resolver(int numFyT, int k, tSol& s, std::vector <bool>& funcionariosUsados, tMatriz const& tiempoFuncionarioTrabajo, int& tiempoMinimo,std::vector <int >const & acum) {
	...
	for (int i = 0; i < numFyT; i++) {
		...
		if (!funcionariosUsados[i]) {
			...
			else { // Sigue completando la solucion si es posible mejorarla
				if(acum[k+1] + s.sumaTiempo < tiempoMinimo ) resolver(numFyT, k + 1, s, funcionariosUsados, tiempoFuncionarioTrabajo, tiempoMinimo, acum);
			}
...
bool resuelveCaso() {
	...
	// Calculo del vector de minimos
	std::vector <int > acum(numFyT);
	int mimTiempoFuncionario;
	for (int i = 0; i < numFyT; ++i) {
		mimTiempoFuncionario = tiempoFuncionarioTrabajo[i][0];
		for (int j = 1; j < numFyT; ++j) if (mimTiempoFuncionario > tiempoFuncionarioTrabajo[i][j]) mimTiempoFuncionario = tiempoFuncionarioTrabajo[i][j];
		acum[i] = mimTiempoFuncionario;
	}
	// Calculo del vector de acumulados
	for (int i = (int)acum.size() - 1; i > 0; --i)  acum[i - 1] += acum[i];

	resolver(numFyT, 0, s, funcionariosUsados, tiempoFuncionarioTrabajo, tiempoMinimo, acum);
